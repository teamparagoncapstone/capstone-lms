...
async function getStudentQuizHistoryByGrades() {
  try {
    ...
    const filteredQuizHistory = quizHistory.filter(item => item.Student.grade === 'GradeOne');
    ...
    const groupedHistory: GroupedHistory = filteredQuizHistory.reduce((acc, curr) => {
      ...
      if (!acc[studentId].modules[moduleId]) {
        acc[studentId].modules[moduleId] = {
          ...
          totalScore: curr.score, // Set totalScore to current score
          quizzes: [curr], // Initialize quizzes with the current quiz
        };
      } else {
        // If module already exists, just push the current quiz
