...
export async function GET() {
  try {
    // Fetch voice exercise scores for Grade One
    const voiceExercises = await prisma.voiceExcercisesHistory.findMany({
      where: { VoiceExcercises: { grade: 'GradeOne' } }, // Assuming 'GradeOne' is an enum value
      select: { score: true },
    });
    ...
    // Fetch quiz scores for Grade One
    const quizHistory = await prisma.studentQuizHistory.findMany({
      where: { Question: { grade: 'GradeOne' } }, // Filter by the grade level
      select: { score: true },
    });
