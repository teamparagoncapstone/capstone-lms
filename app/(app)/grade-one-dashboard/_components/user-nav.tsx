...
import { StudentProfile } from "../../../../components/forms/user-profile-stundent";
...
export function UserNav() {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        className="w-full lg:w-60 overflow-auto"
        align="end"
        forceMount
      >
        <DropdownMenuLabel className="font-normal">
          <div className="flex flex-col space-y-4">
            <p className="flex flex-col text-lg font-medium leading-none text-center">
              {session?.user?.name}
            </p>
            ...
            <p className="text-sm space-y-4 leading-none text-muted-foreground">
              <Badge className="flex flex-col" variant="destructive">
                {" "}
                {session?.user?.role}
              </Badge>
            </p>
